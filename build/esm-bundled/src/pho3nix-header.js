...
if(this._needsShimAdoptedStyleSheets){this._needsShimAdoptedStyleSheets=!1;this.constructor._styles.forEach(s=>{const style=document.createElement("style");style.textContent=s.cssText;this.renderRoot.appendChild(style)})}}/**
   ...
   */LitElement.finalized=!0;/**
                              ...
                              * @nocollapse
                              */LitElement.render=render$2;var litElement={LitElement:LitElement,defaultConverter:defaultConverter,notEqual:notEqual,UpdatingElement:UpdatingElement,customElement:customElement,property:property,query:query,queryAll:queryAll,eventOptions:eventOptions,html:html,svg:svg,TemplateResult:TemplateResult,SVGTemplateResult:SVGTemplateResult,supportsAdoptingStyleSheets:supportsAdoptingStyleSheets,CSSResult:CSSResult,unsafeCSS:unsafeCSS,css:css},__decorate=void 0||function(decorators,target,key,desc){var c=arguments.length,r=3>c?target:null===desc?desc=Object.getOwnPropertyDescriptor(target,key):desc,d;if("object"===typeof Reflect&&"function"===typeof Reflect.decorate)r=Reflect.decorate(decorators,target,key,desc);else for(var i=decorators.length-1;0<=i;i--)if(d=decorators[i])r=(3>c?d(r):3<c?d(target,key,r):d(target,key))||r;return 3<c&&r&&Object.defineProperty(target,key,r),r};function isAnchor(el){return el instanceof HTMLAnchorElement};let Pho3nixHeader=class Pho3nixHeader extends LitElement{constructor(){super(...arguments);this.title="";this.selected=0;this.logo="";this.alt="";this.sticky=!1;this.collapsed=!1;this.previousPageYOffset=0;this.onScroll=()=>{const wrapper=this.wrapper;if(this.collapsed){if(this.isMenuOpen)this.toggleMenu();return};if(this.sticky&&wrapper){if(this.previousPageYOffset<window.pageYOffset){wrapper.classList.remove("sticky");wrapper.classList.add("hide")}else{wrapper.classList.remove("hide");wrapper.classList.add("sticky")}}this.previousPageYOffset=window.pageYOffset};this.onClick=event=>{const slotted=this.slotted,target=event.target;if(target&&slotted&&slotted.includes(target)){this.selected=slotted.indexOf(target)}}}static get styles(){return[css`
      ...
      `]}get wrapper(){return this.renderRoot&&this.renderRoot.querySelector("#wrapper")}get openButton(){return this.renderRoot&&this.renderRoot.querySelector("#openButton")}get slotted(){const slot=this.renderRoot&&this.renderRoot.querySelector("slot"),assignedNodes=slot&&slot.assignedNodes();return assignedNodes&&assignedNodes.filter(isAnchor)}connectedCallback(){super.connectedCallback();window.addEventListener("scroll",this.onScroll)}disconnectedCallback(){super.disconnectedCallback();window.removeEventListener("scroll",this.onScroll)}get isMenuOpen(){const{wrapper}=this;return wrapper&&wrapper.classList.contains("sticky")}toggleMenu(){const{wrapper,openButton}=this;if(wrapper&&openButton){wrapper.classList.contains("preload")&&wrapper.classList.remove("preload");wrapper.classList.toggle("sticky");wrapper.classList.toggle("hide");openButton.classList.toggle("close")}}render(){return html`
   <style>
